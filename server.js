...
app.get('/api/hourlySalesData/:startDate', (req, res) => {
    let sql = `SELECT Product, Cat, Description as Department, Quantity as Qty, (PackCost / PackSize) as Cost, (Price * Quantity) as Sales, Amount as Refund, DATE_FORMAT(TillDate, '%y-%m-%d') as TillDate, TIME_FORMAT(TillTime, '%H:%m') as TillHour, TillTime FROM(
        (SELECT Description, CategoryID as Cat FROM Category) as a
         JOIN
