...
FROM (
     ...
     (SELECT ti.TillItemID,
      ...
      FROM TillItem ti
               ...
               LEFT JOIN Category c ON p.CategoryID = c.CategoryID
               LEFT JOIN RefundItem ri ON ti.RefundID = ri.RefundID
      WHERE t.TillDate >= '2020-04-15'
        AND t.TillDate <= '2020-04-15'
      ORDER BY t.TillID, ti.TillItemID) as b
     ON b.CategoryID = a.Cat
         LEFT JOIN pricemark pm ON pm.AssocProdID = ProdID
    ) group by AssocProdID;

SELECT *
FROM pricemark pm
         JOIN tillitem ti on pm.AssocProdID = ti.ProdID
where ti.Date >= '2020-04-08'
  AND ti.Date <= '2020-04-08';