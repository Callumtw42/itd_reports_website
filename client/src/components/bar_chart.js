...
export function useStackedBarChart(
  ...
  function formatChartData(
    ...
    let stacks = d.getUniqueValues(data, z);
    let stackNames = d.getUniqueValues(data, idToName(z));
    ...
    let dataSets = stacks.map((stack, index) => {
      ...
      return {
        ...
        data: x.map((t) => {
          let atHour = d.getElementsWithValue(data, 'TillHour', t);
          let withCategory = d.getElementsWithValue(atHour, z, stack);
          return d.notEmpty(withCategory) ? d.getColumn(withCategory, y).reduce((acc, n) => acc + n) : 0;
        }),
